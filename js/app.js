...
if (document.querySelector('.slider-people__body')) {
	new Swiper('.slider-people__body', {
		//observer: true,
		//observeParents: true,
		...
		// Arrows
		navigation: {
			nextEl: '.ps-slider-people .slider-arrow_next',
			prevEl: '.ps-slider-people .slider-arrow_prev',
		},
		breakpoints: {
...
if (document.querySelector('.slider-one__body')) {
	new Swiper('.slider-one__body', {
		observer: true,
		observeParents: true,
		slidesPerView: 1,
		spaceBetween: 10,
...
if (document.querySelector('.slider-clients-love__body')) {
	new Swiper('.slider-clients-love__body', {
		observer: true,
		observeParents: true,
		slidesPerView: 3,
		spaceBetween: 32,
...
gsap.from(".header__logo", {duration: 1.5, opacity: 0, scale: 0.3, ease:"back", repeat: 2, yoyo: true});

gsap.from(".fsa", {duration: 1, opacity: 0, y: 150, delay: 3.5});
...
function ActiveColumn() {
    let col = document.querySelectorAll('.sessions-column');

    for (let i=0; i < col.length; i++) {
        col[i].addEventListener("mouseover", function () {
            for (var j=0; j < col.length; j++) {
                if (j === i) { col[j].classList.add('active-col')}
                else {col[j].classList.remove('active-col')};
            }
        });
    };
...
columns.forEach(column => {
    column.addEventListener('click', function() {
        columns.forEach(clm => clm.classList.remove ('active'));
        this.classlist.add('active');
    });
});

document.addEventListener('scroll', function (){
    let par = document.querySelector('.FastSimple__bg');
    if (scrollY < 210) {par.style.transform = 'translate3d(0, '+scrollY/3+'px, 0)'}
});

/*
$(window).scroll(function(event) {
    var s=0-$(this).scrollTop()/3;
    $('.FastSimple__bg').css('transform','translate3d(0, '+s+'px, 0)');
});
 ...
 //          const parallaxHeight = parallax.offsetHeight;
  ...
  //          threshold: thresholdSets
   //     });
    ...
    //        bookSession.style.cssText = `transform: translate(0%,-${scrollTopProcent / 9}%);`;
    //        parallaxContainer.parentElement.style.cssText = `transform: translate(0%,-${scrollTopProcent / 3}%);`;
  //      }
